...
App-page navigation now switches to Downloader, Updates, and Settings immediately, then defers each page's heavier refresh work until after the destination page is visible so sidebar page changes no longer block on those refreshes.
...
Per-title saved-source settings now also persist update behavior such as notify-only, auto-download, ignore-update-checks, and an optional latest-chapters auto-download limit.
Library detail-page remote chapter checks now honor per-title auto-download settings so detected updates can start immediately from the detail view instead of only during scheduled checks.
Downloader recent activity now stores failure reasons and surfaces retry and site-authorization actions directly from failed entries.
Library categories now support independent built-in section visibility toggles for New, Active Downloads, Bookmarked, Continue Reading, Needs Update, and Completed, while empty smart sections stay hidden until they contain titles.
Settings now include Library Health Tools that scan for invalid series folders, empty chapter folders, missing saved sources, missing cached covers, orphaned metadata, and orphaned thumbnails, with safe cleanup actions for removable issues.
Shared button styles now use stronger disabled-state visuals so unavailable actions are easier to distinguish throughout the app.
...
Manual downloads and saved-source updates now share one app-wide scheduler with an explicit eight-job global queue limit while still preventing duplicate jobs for the same title or source across the Downloader and Updates pages.
Manual downloads and saved-source updates now persist resumable job metadata in download history and SQLite so queued or interrupted work is restored on the next app launch with its target path, selected chapters, and resolved title when available.
Restored downloader jobs stay visible in the Downloader page immediately after launch, and their resumable queue state remains in recent activity until the job completes, fails, or is explicitly cancelled.
DownloadService now delegates runtime job state, global queue coordination, and download history/resume bookkeeping into dedicated downloader helper modules so the service mainly orchestrates active jobs and download execution paths.
...
The viewer now supports per-chapter scene bookmarks that save exact image positions with optional notes and cropped thumbnails so specific moments inside long chapters can be revisited quickly.
Library detail now surfaces saved scene counts per chapter, offers a scene-only chapter filter, and opens saved scenes directly back into the viewer at the stored position.
The viewer now includes focused reading-session controls with hide-UI mode, a quiet session overlay, an upgraded progress mini-map, clickable saved-scene anchors, and extended keyboard shortcuts for long reading sessions.
The viewer now uses a centered floating icon toolbar that appears on hover, keeps chapter controls compact, and preserves the chapter selector popup while it is open.
...
Startup app-update dialogs now show the release patch notes from GitHub inside the modal so users can review changes before installing.
Startup and store initialization now use explicit SQLite schema versioning so database changes can be applied safely and consistently across upgrades.
Shared HTTP work now routes through one requests-based helper so update checks, cover loading, thumbnail downloads, and session validation use the same retry, timeout, and logging policy.
Source availability now supports scoped modes so a source can stay fully enabled, be hidden from Discover only, or be disabled for both discovery and downloads.
The source availability control now lives in Settings, while the Discover page no longer exposes source-disable actions directly.
Settings source rows can now also mark one Discover-capable source as the default so the Discover page opens on that provider first whenever it is available.
Source availability selectors now use dedicated popup list views and explicit fonts so their dropdown menus render cleanly above surrounding rows without Qt point-size warnings.
Application startup now sets an explicit default Qt font so combo-box popups and similar controls inherit a stable point-sized font instead of relying on platform defaults.
Discover page loading feedback now stays minimal before the first catalog response, hides result and empty-state labels until real data arrives, and uses lighter inline loading copy instead of a heavy card-like banner.
...
Viewer auto-skip now uses more conservative panel-gap detection and panel-aligned target selection so dialogue bands stay attached to the intended panel, short and medium panels are centered more reliably across zoom levels, and tall panels advance in ordered reading steps instead of landing between panels.
Viewer async image preview and decode height corrections now preserve the current packed reading position when content above the viewport resizes, preventing late auto-skip reading sessions from jumping upward unexpectedly.
...
Detail-page online chapter checks now suppress automatic site-authorization popups during background refreshes, and stale remote lookup completions are ignored once the detail page is no longer the active view.
Detail-page chapter rows now parent their selector and action toolbuttons before visibility updates so rebuilding the chapter list after online checks does not create stray top-level button windows or steal focus.
Chapter deletion now only clears progress/new markers after folder removal succeeds, preserves metadata for failed deletions, and persists bookmark cleanup when deleted chapters had saved bookmarks.
Progress saves now use millisecond timestamps, and latest-progress lookups use deterministic ordering so continue/resume actions no longer depend on second-level SQLite timestamp ties.
Startup now prewarms the SQLite schema in the background and schedules the first reusable main-thread SQLite connection right after the main window appears so the first database-backed interaction feels faster.
Ctrl+K search now debounces fuzzy ranking updates and only flushes immediately for explicit actions like Enter or command completion, reducing repeated full-library scoring while typing.
Library scans now skip inline auto-thumbnail generation when no cached/custom cover exists and fall back to the first chapter image instead, reducing synchronous image-processing work during large scans.
Scrapers can now provide an optional `fetch_cover()` path for protected covers, and shared thumbnail/detail cover loading reuses those custom cover fetchers when needed so saved covers render correctly across the app.
Settings now show per-source reliability badges directly in the Sources list, allow one-click live status tests, and route blocked checks into the existing site-authorization browser flow without extra confirmation prompts.
Settings source rows now use per-source mode dropdowns instead of a single enabled checkbox so discovery-only disables and full disables are handled explicitly.
Discovery catalog loading is now deferred until the Discover page is actually opened, reducing unnecessary remote work during startup.
Startup update result persistence now batches app-settings writes in a single transaction, and automatic startup checks are delayed so the app no longer feels blocked immediately after launch.
Discovery cover cards now treat failed remote fetches or decode failures as terminal for the current card instance so broken covers stop retrying and re-logging while the user scrolls.
Discovery cover application now batches loaded-cover UI updates behind a short timer so large MVLEMPYR result sets do not flood the main thread with back-to-back pixmap work.
Startup saved-title update checks now defer and retry later when Discover is the active page and its catalog is still loading, preventing background update scans from competing with the initial discovery load.
Site authorization requests now launch through a guarded lazy import path so the in-app browser can open without duplicate dialogs or main-window state restoration side effects.
The in-app authorization browser now filters expected low-signal Chromium console noise and delays profile teardown until after page cleanup so session dialogs are less noisy during normal use.
The app now includes a Ctrl+K global search and command palette for quickly opening titles, jumping between pages, and running common reader or downloader actions from the keyboard.
...
switches to the page first, then schedules open-triggered refreshes on a single-shot timer so navigation feels immediate
throttles repeated open-triggered refreshes for a short window so quick tab flips do not immediately restart full saved-title scans and remote checks
...
OK Per-chapter scene bookmarks with notes and cropped scene thumbnails
OK Focused reading-session tools with mini-map scene anchors, hideable reader chrome, and extended keyboard navigation
...
OK Downloader page defers recent-activity rebuilds until after the page becomes visible and throttles repeated open-triggered refreshes
OK Updates page defers open-triggered refreshes until after the page becomes visible and throttles repeated open-triggered checks
...
OK Settings page defers scraper-reliability badge refreshes until after the page becomes visible
...
The app now supports both webtoons and webnovels through scraper content-type tagging, including a new MVLEMPYR webnovel scraper, browser-backed fallback fetching for blocked chapter pages, and a helper-process path that keeps hidden browser work off the main UI process when possible.
Webnovel downloads are stored as structured chapter.json payloads, the library recognizes text-only chapter folders, and the viewer now has a dedicated text-reading mode with typography/color settings, a side progress rail, full-chapter keyboard paging, and text-specific bookmark flows.
The text reader now supports continuous multi-chapter reading with a moving memory window that appends, prepends, and trims nearby chapters while preserving per-chapter progress, chapter-relative progress rails, and bookmark jumps.
Detail pages now expose a Saved action that opens all saved scenes or bookmarks across every chapter for the current title, while the reader hover toolbar exposes save/open bookmark actions for novels and text-reader controls for progress visibility and typography settings.
Discover now includes an MVLEMPYR discovery provider, and the discovery page can hand the shared browser fetcher into providers so protected catalogs can reuse the same session-aware cover and HTML fetch paths when needed.
MVLEMPYR discovery now stabilizes noisy catalog cards by deriving discovery titles from novel slugs and rejecting obviously shared section covers when they do not match the target novel metadata.
...
Custom scraper chapter downloads now reuse one job-scoped ThreadPoolExecutor across chapters instead of recreating a fresh pool per chapter, reducing thread churn and improving connection reuse during long downloads.
Updates-page result rendering, downloader recent-activity refreshes, and library update-control refreshes now batch repeated UI rebuilds behind short timers so starting updates during active downloads no longer rebuilds large card grids on every intermediate signal.
Main-window page navigation now switches pages before kicking off page-owned refresh work, while Downloader, Updates, and Settings schedule their expensive open-time refresh paths on single-shot timers so navigation feels immediate and repeated page flips reuse recent results instead of rebuilding everything inline.
MainWindow orchestration is now split across focused helper controllers for navigation, sidebar/download indicator state, chapter-loading overlays, authorization flow, and library-update scheduling so the window mostly composes pages and shared services.
The shared download queue now exposes an explicit MAX_CONCURRENT_DOWNLOAD_JOBS constant and defaults to eight parallel jobs instead of hardcoding the limit inside the queue constructor.
Library update-control syncing now bulk-loads settings rows for visible cards instead of issuing repeated per-card store lookups, and manual-download live polling now only refreshes cards for currently active downloads.
Multi-delete now removes selected library cards from the UI immediately after confirmation, deletes their folders in a background worker, and reconciles failed deletions back into the library on the follow-up reload instead of blocking the UI thread during folder removal.
